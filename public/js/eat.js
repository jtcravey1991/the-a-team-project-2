...
let eatChart = new Chart(eatLogChart, {
    type: "bar",
    data: {
        labels: [],
        datasets: [{
...
function addEatValue(){
    ...
    if (eatValue == 3 || eatValue == 4) {
        document.getElementById("eatProgress").innerHTML = "Keep up the healthy eating!"
    } else if(eatValue == 5){
        document.getElementById("eatProgress").innerHTML = "Wow! Amazing!"
    } else{
        document.getElementById("eatProgress").innerHTML = "Let's try to eat better!"
    };
    eatChart.update();
